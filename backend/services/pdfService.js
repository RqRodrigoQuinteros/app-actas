    ...
    try {
      ...
      const req = mod.get(url, { timeout }, (res) => {
        ...
        res.on('end', () => {
          ...
          setTimeout(() => imageCache.delete(url), IMAGE_CACHE_TTL);
          resolve(result);
        });
      });
...
function buildPdfHeader(logoMinisterio, logoCordoba, membrete) {
  const headerLogoMinisterio = logoMinisterio ? `<img src="${logoMinisterio}" style="height: 40px;" />` : '';
  const headerLogoCordoba = logoCordoba ? `<img src="${logoCordoba}" style="height: 40px;" />` : '';
  return membrete
    ? `<img src="${membrete}" style="height: 50px; max-width: 100%; object-fit: contain;" />`
...
async function renderPdf({ html, logoMinisterio, logoCordoba, membrete, margin = { top: '25mm', bottom: '15mm', left: '20mm', right: '20mm' } }) {
...
async function withRetry(fn, maxRetries = 3, label = 'PDF') {
  let lastError;
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(`[${label}] Intento ${attempt}/${maxRetries}`);
      return await fn(attempt);
    } catch (err) {
      lastError = err;
      console.log(`[${label}] ERROR intento ${attempt}/${maxRetries}:`, err.message);
...
const SECCIONES_POR_TIPOLOGIA = {
  quirurgicos: [
    'conclusion_inspeccion', 'registros', 'datos_generales', 'consultorios_externos',
    'consultorios_salud_mental', 'la_institucion_posee', 'sector_internacion',
    'enfermeria', 'area_quirurgica', 'obstetricia', 'laboratorio', 'guardia',
    'uco', 'uti', 'utin', 'hemodinamia', 'hospital_dia',
    'quirurgicos_inscripcion', 'quirurgicos_direccion_funcionamiento',
    'quirurgicos_enfermeria', 'quirurgicos_area_internacion',
    'quirurgicos_equipamiento', 'quirurgicos_esterilizacion'
  ],
  hemodialisis: [
    'conclusion_inspeccion', 'registros', 'datos_generales', 'sector_internacion',
    'enfermeria', 'hemodialisis_direccion_funcionamiento',
    'hemodialisis_analisis_agua', 'hemodialisis_serologia'
  ],
  estetica: [
    'conclusion_inspeccion', 'registros', 'datos_generales',
    'estetica_inscripcion', 'estetica_direccion_funcionamiento', 'estetica_consultorios'
  ],
  opticas: [
    'conclusion_inspeccion', 'registros', 'opticas_local', 'opticas_taller', 'opticas_gabinete_contactologia'
  ],
  centambulatorios: [
    'conclusion_inspeccion', 'registros', 'datos_generales', 'consultorios_externos',
    'sector_internacion', 'enfermeria', 'centamb_inscripcion',
    'centamb_direccion_funcionamiento', 'centamb_esterilizacion'
  ],
  clinica: [
    'conclusion_inspeccion', 'registros', 'datos_generales', 'consultorios_externos',
    'consultorios_salud_mental', 'la_institucion_posee', 'radiofisica',
    'sector_internacion', 'enfermeria', 'area_quirurgica', 'obstetricia',
    'laboratorio', 'guardia', 'uco', 'uti', 'utin', 'hemodinamia', 'hospital_dia'
  ],
};
...
async function generarActaPDF(acta, logoMinisterioBase64, logoCordobaBase64, membreteBase64) {
  return withRetry(async (attempt) => {
    ...
    const htmlFinal = template({
      ...
      esEquipamiento,
      dia_texto, mes_texto, anio_texto,
    ...
    return renderPdf({ html: htmlFinal, logoMinisterio: logoMinisterioBase64, logoCordoba: logoCordobaBase64, membrete: membreteBase64 });
  }, 3, 'PDF');
}
...
async function generarInformeGeriatricoPDF(datos, logoMinisterioBase64, logoCordobaBase64, membreteBase64) {
  return withRetry(async () => {
    ...
    return renderPdf({
      html: htmlFinal,
      logoMinisterio: logoMinisterioBase64,
      logoCordoba: logoCordobaBase64,
      membrete: membreteBase64,
